...
export function ReceiptPrint({ sale, onClose }: ReceiptPrintProps) {
  ...
  const handlePrint = () => {
    setTimeout(() => {
      ...
      clonedReceipt.classList.add('print-body-visible');

      window.print();

      setTimeout(() => {
        clonedReceipt.remove();
        bodyChildren.forEach((child) => {
